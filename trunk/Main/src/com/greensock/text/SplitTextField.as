 ...
 * 		<li>Positioning may incorrect when non-standard antialiasing is used (like "anti-alias for readability")</li>
 ...
 */	
	public class SplitTextField extends Sprite {
		...
		/** @private TextFields created in the Flash IDE have a different 2-pixel gutter than other TextFields! **/
		private var _extraOffset:int;
		...
		public static function split(source:TextField, spitType:String="characters", container:DisplayObjectContainer=null, offset:Point=null):Array {
			...
			for (line = 0; line < linesTotal; line++) {
				...
				if (format.align == "left") {
					...
					x = offset.x - 2 + (source.width - lineMetrics.width) * 0.5;
				} else {
					x = offset.x - 4 + (source.width - lineMetrics.width);
				}
		...
		/** @private **/
		private function setExtraOffset():void {
			var oldFormat:TextFormat = _source.getTextFormat(0, 1);
			var newFormat:TextFormat = _source.getTextFormat(0, 1);
			newFormat.align = TextFormatAlign.LEFT;
			_source.setTextFormat(newFormat, 0, 1);
			var metrics:TextLineMetrics = _source.getLineMetrics(0);
			_extraOffset = ((metrics.x == 0 && _source.embedFonts) || metrics.x > 2) ? 2 : 0;
			_source.setTextFormat(oldFormat, 0, 1);
		}
		...
		private function update():void {
			...
			this.textFields = split(_source, _splitType, this, new Point(-_regOffsetX - _extraOffset, -_regOffsetY - _extraOffset));
		}
