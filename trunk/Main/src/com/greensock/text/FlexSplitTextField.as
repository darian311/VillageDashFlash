	...
	public class FlexSplitTextField extends Sprite {
		...
		private function findSourceTF():void {
			...
			if (_sourceTF == null) {
				...
				var oldFormat:TextFormat = _sourceTF.getTextFormat(0, 1);
				var newFormat:TextFormat = _sourceTF.getTextFormat(0, 1);
				newFormat.align = TextFormatAlign.LEFT;
				_sourceTF.setTextFormat(newFormat, 0, 1);
				var metrics:TextLineMetrics = _sourceTF.getLineMetrics(0);
				_extraOffset = ((metrics.x == 0 && _sourceTF.embedFonts) || metrics.x > 2) ? 2 : 0;
				_sourceTF.setTextFormat(oldFormat, 0, 1);
			}
		}
		...
		private function update():void {
			...
			this.textFields = split(_sourceTF, _splitType, this, new Point(-_regOffsetX - _extraOffset, -_regOffsetY - _extraOffset));
		}
